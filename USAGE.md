...
```bash
python3 predict.py 主队 客队
# 例如
python3 predict.py 辽宁铁人 北京国安
python3 predict.py 山东泰山 上海海港
```

程序会自动补全赛前 λ 并输出**比分概率 TOP 8、胜平负、大小球、双方进球**等，无需任何文件。
> 注意：第一个参数若是已存在的文件名，会走"文件预测"模式；给两个队名才是赛前快捷预测。

### 方式二：JSON 文件（可附带赔率，才会给投注建议）
